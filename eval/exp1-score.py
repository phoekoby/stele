...
ARM_ORDER = ["plain","stele","cat10","cat40","cat159"]
...
for t in tasks:
    row=""
    plain_rate = sum(by_task_arm[(t,'plain')])/max(1,len(by_task_arm[(t,'plain')]))
    stele_rate = sum(by_task_arm[(t,'stele')])/max(1,len(by_task_arm[(t,'stele')]))
    is_disc = plain_rate < 0.5 and stele_rate >= 0.5
