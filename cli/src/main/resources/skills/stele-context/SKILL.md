---
name: stele-context
description: Use before implementing or changing a feature in this codebase. Fetches the product rules, domain concepts, and cross-language code map for the file or concept you are touching (via the Stele MCP server), so the change respects product invariants and lands in the right place. Trigger when the user describes a feature in business/domain language, or before editing a non-trivial file.
---

# Stele context

This repo is indexed by Stele into a concept graph — code ↔ docs ↔ **product rules**. Before you write
code, pull the relevant context from the `stele` MCP server. The rules it returns are team invariants
that are NOT visible in the code; honoring them is the point.
...
1. Read the **product rules** and treat them as hard constraints. Code that compiles but breaks one of
   them is wrong.
