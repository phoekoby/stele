...
- **OSS / локально:** **SQLite** (узлы/рёбра как таблицы + recursive CTE для обхода) + **sqlite-vec** для эмбеддингов. Ноль инфры — нативка в jar (`sqlite-jdbc`), `java -jar` и поехали; критично для adoption. *Граф-нативная альтернатива: **KùzuDB** (встраиваемый граф с вектор-индексом) — красиво, но SQLite проще и привычнее команде.*
